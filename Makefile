...
.PHONY: help env front-install front-build front-dev sync status docker-build docker-up docker-down docker-logs clean
...
front-install: ## Install the frontend dependencies
	cd frontend && npm install

front-build: ## Build the frontend (output in frontend/dist, served by the app)
	cd frontend && npm run build

front-dev: ## Start the Vite dev server (proxies the API to localhost:8000)
...
clean: ## Remove the virtual environment, build artifacts, and compiled Python files
