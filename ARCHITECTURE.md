...
- `ACCESS_CONTROL` (true by default) / `ACCESS_CACHE_TTL` (300s): per-user wiki access
  control when sign-in is enabled (see [Access control](#access-control-1))
...
`_allowed_scope_keys(request)` returns `None` when access control is off (no filtering) or
the resolved set otherwise. `POST /api/chat` passes it to `build_context(...,
allowed_scope_keys=...)`, which drops pages outside the set **before** they ever reach the
model. `GET /api/status` uses it to report `pages_accessible` alongside `pages_indexed`.
...
| `GET /api/status` | `sync_manager.status()` + `auth_enabled`, `access_control`, and (under access control) `pages_accessible` |
...
- **Access control via the app token** (membership + visibility) rather than the user's
  OAuth token: keeps the OAuth scope minimal (`read_user`), stores no user tokens, and stays
  fresh via a short cache — at the cost of not covering exotic per-feature `wiki_access_level`
  overrides. It **fails closed** so GitLab errors never leak content.
